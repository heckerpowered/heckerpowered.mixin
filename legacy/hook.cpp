...
namespace k_hook
{
	...
	NTSTATUS modify_trace_settings(trace_type type)
	{
		...
		const NTSTATUS status = NtTraceControl(type, property, PAGE_SIZE, property, PAGE_SIZE, &length);
	...
	bool initialize(fptr_call_back fptr)
	{
		...
		// 获取系统基址
		const unsigned long long ntoskrnl = k_utils::get_module_address("ntoskrnl.exe", nullptr);
	...
	bool stop()
	{
		const bool result = NT_SUCCESS(modify_trace_settings(stop_trace)) && NT_SUCCESS(modify_trace_settings(start_trace));
