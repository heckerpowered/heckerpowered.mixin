...
NMD_ASSEMBLY_API bool _nmd_parse_number(const char* string, int64_t* p_num, size_t* p_num_digits)
{
	...
	{
		const char c = s[i];
		num += (c <= '9') ? (static_cast<long long>(c) - '0') : (static_cast<long long>(10) + c - 'a');
		if (i < num_digits - 1)
		{
...
NMD_ASSEMBLY_API bool nmd_decode_x86(const void* buffer, size_t buffer_size, nmd_x86_instruction* instruction, NMD_X86_MODE mode, uint32_t flags)
{
	...
	const size_t remaining_valid_bytes = (static_cast<size_t>(NMD_X86_MAXIMUM_INSTRUCTION_LENGTH) - instruction->num_prefixes);
...
NMD_ASSEMBLY_API void _nmd_append_relative_address8(_nmd_string_info* const si)
{
	...
	else
	{
		...
		if (si->instruction->mode == NMD_X86_MODE_64)
			...
			n = (uint64_t)(static_cast<uint64_t>((int32_t)(si->runtime_address + si->instruction->length)) + (int8_t)(si->instruction->immediate));
	...
	if (si->instruction->disp_mask != NMD_X86_DISP_NONE && (si->instruction->displacement != 0 || *(si->buffer - 1) == '['))
	{
		...
		if (si->instruction->modrm.fields.rm == 0b101 && si->instruction->mode == NMD_X86_MODE_64 && si->instruction->modrm.fields.mod == 0b00 && si->runtime_address != NMD_X86_INVALID_RUNTIME_ADDRESS)
		{
			if (si->instruction->prefixes & NMD_X86_PREFIXES_ADDRESS_SIZE_OVERRIDE)
				_nmd_append_number(si, (uint64_t)((int64_t)(static_cast<long long>(si->runtime_address) + static_cast<long long>(si->instruction->length))) + (int64_t)si->instruction->displacement);
			else
...
NMD_ASSEMBLY_API void nmd_format_x86(const nmd_x86_instruction* instruction, char* buffer, uint64_t runtime_address, uint32_t flags)
{
	...
	if (flags & NMD_X86_FORMAT_FLAGS_BYTES)
	{
		...
		const size_t num_padding_bytes = instruction->length < NMD_X86_FORMATTER_NUM_PADDING_BYTES ? (static_cast<size_t>(NMD_X86_FORMATTER_NUM_PADDING_BYTES) - instruction->length) : 0;
